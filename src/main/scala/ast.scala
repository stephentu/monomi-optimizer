...
trait Node extends PrettyPrinters {
  ...
  // recursive- NOT the same as copyWithContext(null)
  def withoutContext: Node

  // simply a helper
  @inline def withoutContextT[N <: Node]: N =
    withoutContext.asInstanceOf[N]
...
case class SelectStmt(projections: Seq[SqlProj],
  ...
  def withoutContext =
    SelectStmt(
      projections.map(_.withoutContextT[SqlProj]),
      relations.map(_.map(_.withoutContextT[SqlRelation])),
      filter.map(_.withoutContextT[SqlExpr]),
      groupBy.map(_.withoutContextT[SqlGroupBy]),
      orderBy.map(_.withoutContextT[SqlOrderBy]),
      limit)
...
case class ExprProj(expr: SqlExpr, alias: Option[String], ctx: Context = null) extends SqlProj {
  def copyWithContext(c: Context) = copy(ctx = c)
  def withoutContext = copy(expr = expr.withoutContextT[SqlExpr], ctx = null)
...
case class Or(lhs: SqlExpr, rhs: SqlExpr, ctx: Context = null) extends Binop {
  ...
  def copyWithContext(c: Context) = copy(ctx = c)
  def withoutContext =
    copy(lhs = lhs.withoutContextT[SqlExpr],
         rhs = rhs.withoutContextT[SqlExpr], ctx = null)
...
case class In(elem: SqlExpr, set: Seq[SqlExpr], negate: Boolean, ctx: Context = null) extends SqlExpr {
  ...
  def copyWithContext(c: Context) = copy(ctx = c)
  def withoutContext =
    copy(elem = elem.withoutContextT[SqlExpr],
         set = set.map(_.withoutContextT[SqlExpr]), ctx = null)
...
case class Exists(select: Subselect, ctx: Context = null) extends SqlExpr {
  ...
  def copyWithContext(c: Context) = copy(ctx = c)
  def withoutContext = copy(select = select.withoutContextT[Subselect], ctx = null)
...
case class FieldIdent(qualifier: Option[String], name: String, symbol: Symbol = null, ctx: Context = null) extends SqlExpr {
  ...
  def withoutContext = copy(symbol = null, ctx = null)
...
case class Subselect(subquery: SelectStmt, ctx: Context = null) extends SqlExpr {
  ...
  def toCPP = throw new RuntimeException("Should not happen")
  def copyWithContext(c: Context) = copy(ctx = c)
  def withoutContext = copy(subquery = subquery.withoutContextT[SelectStmt], ctx = null)
  def gatherFields = Seq.empty
...
case class AggCall(name: String, args: Seq[SqlExpr], ctx: Context = null) extends SqlAgg {
  ...
  def copyWithContext(c: Context) = copy(ctx = c)
  def withoutContext = copy(args = args.map(_.withoutContextT[SqlExpr]), ctx = null)
...
case class CaseExprCase(cond: SqlExpr, expr: SqlExpr, ctx: Context = null) extends SqlExpr {
  ...
  def copyWithContext(c: Context) = copy(ctx = c)
  def withoutContext =
    copy(cond = cond.withoutContextT[SqlExpr],
         expr = expr.withoutContextT[SqlExpr], ctx = null)
...
case class CaseExpr(expr: SqlExpr, cases: Seq[CaseExprCase], default: Option[SqlExpr], ctx: Context = null) extends SqlExpr {
  ...
  def copyWithContext(c: Context) = copy(ctx = c)
  def withoutContext =
    copy(expr = expr.withoutContextT[SqlExpr],
         cases = cases.map(_.withoutContextT[CaseExprCase]),
         default.map(_.withoutContextT[SqlExpr]), ctx = null)
  override def isLiteral =
    expr.isLiteral &&
...
case class CaseWhenExpr(cases: Seq[CaseExprCase], default: Option[SqlExpr], ctx: Context = null) extends SqlExpr {
  ...
  def copyWithContext(c: Context) = copy(ctx = c)
  def withoutContext =
    copy(cases = cases.map(_.withoutContextT[CaseExprCase]),
         default = default.map(_.withoutContextT[SqlExpr]), ctx = null)
  override def isLiteral =
    cases.filter(x => !x.cond.isLiteral || !x.expr.isLiteral).isEmpty &&
    default.map(_.isLiteral).getOrElse(true)
  override def isRValueLiteral =
...
case class IntLiteral(v: Long, ctx: Context = null) extends LiteralExpr {
  ...
  def copyWithContext(c: Context) = copy(ctx = c)
  def withoutContext = copy(ctx = null)
...
case class JoinRelation(left: SqlRelation, right: SqlRelation, tpe: JoinType, clause: SqlExpr, ctx: Context = null) extends SqlRelation {
  def copyWithContext(c: Context) = copy(ctx = c)
  def withoutContext =
    copy(left = left.withoutContextT[SqlRelation],
         right = right.withoutContextT[SqlRelation],
         clause = clause.withoutContextT[SqlExpr], ctx = null)
...
case class SqlGroupBy(keys: Seq[SqlExpr], having: Option[SqlExpr], ctx: Context = null) extends Node {
  def copyWithContext(c: Context) = copy(ctx = c)
  def withoutContext =
    copy(keys = keys.map(_.withoutContextT[SqlExpr]),
         having = having.map(_.withoutContextT[SqlExpr]), ctx = null)
...
case class SqlOrderBy(keys: Seq[(SqlExpr, OrderType)], ctx: Context = null) extends Node {
  def copyWithContext(c: Context) = copy(ctx = c)
  def withoutContext =
    copy(keys = keys.map { case (e, o) => (e.withoutContextT[SqlExpr], o) }, ctx = null)
...
case class MetaFieldIdent(pos: Int, tpe: DataType, ctx: Context = null) extends SqlExpr {
  ...
  def gatherFields = throw new RuntimeException("error")
  def sqlFromDialect(dialect: SqlDialect) =
    "<meta_field_ident:%s>".format(toString)
}
